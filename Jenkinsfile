...
def server = Artifactory.server 'server1' //server1 is the Server ID given to Artifactory server in Jenkins
...
node {
	...
	stage ('Checkout') {
		checkout scm	
		//checkout([$class: 'GitSCM', branches: [[name: '*/testing']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory']], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/yerriswamykonanki/CICD.git']]])
	}
